...
Phase 1 (consumer). Spec/plan under `.superpowers/sdd/2026-09-23-guided-tours-phase1/`. Driver.js-powered walkthroughs, one `getting-started` shell tour plus one per-page module tour, all consumer-only.
...
- **Progress loading is fail-closed, and that's deliberate.** `fetchProgress()` (`apps/web/lib/tours/progress.ts`) returns `null` on ANY failure (network error, non-2xx, malformed body) — `shouldAutoPlay` treats `progress === null` as "never auto-play, for the rest of this load," never as "treat everything as unseen." This is why the guided-tours feature is safe to ship with its migration not yet applied to Neon: every environment without the table just gets fail-closed silence (verified live — see below), and it's also why the OTHER Playwright specs in `tests/e2e/` stay overlay-free: they never mock `/tours/progress` at all, so the real fetch 404/500s against their mocked backends and no popover ever appears to intercept a click meant for the page under test.
