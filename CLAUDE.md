...
- **Build note.** Web resolves `@wusuq/shared` from `packages/shared/dist`, not a tsconfig path alias to its `src` — the alias was removed because Turbopack can't map a `.js`-suffixed relative specifier (e.g. `./tours.js`, required since `packages/shared` compiles as NodeNext) back to a `.ts` source file. `apps/web`'s `build`/`typecheck`/`test` scripts each prebuild `@wusuq/shared` first; `packages/shared` also has a `dev` watch script (`tsc --watch --preserveWatchOutput`) for local iteration, and `pnpm dev:web` at the repo root runs `pnpm --filter "@wusuq/web..." --parallel dev` so the shared watcher and `next dev` start together. Relative imports **between files inside `packages/shared/src`** need an explicit `.js` extension. **Vercel builds only `apps/web`**, so removing web's shared-prebuild step would break production even if a monorepo-aware CI still passed.
...
- **A failed save is queued, not dropped.** `persist()` always applies the outcome to local `progress` state optimistically and fires the PUT; on failure the row is appended to `localStorage['wusuq_tour_pending']` and retried on the next page load (`flushPending` in `tour-provider.tsx`), merged over the server's rows so a blip can't make a completed tour replay.
