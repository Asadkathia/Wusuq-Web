...
- **`lib/staff-routes.ts` is a security boundary — `staffLoginHref(next)` is the ONLY way to build a login href carrying `next`.** Never hand-concatenate `?next=`. It validates against open redirect: absolute, protocol-relative (`//`), **backslash (`/\evil.test` — browsers treat leading `\` as `/` for http/https, so this resolves to a foreign origin and `router.replace` hard-navigates)**, percent-encoded `//`, and `javascript:`; drops consumer-side and self-referential targets; fails closed on non-string. A backslash bypass shipped in the first draft and was caught in review — don't weaken these guards.
