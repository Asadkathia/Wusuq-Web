...
- **⚠️ Migration `20260923000000_add_user_tour_progress` is written but NOT YET APPLIED to Neon.** The Render API `buildCommand` runs `prisma:migrate:deploy`, so the next API deploy WILL apply it (additive, safe). Until the table exists, `GET /tours/progress` 500s and the web fails closed — no auto-play, no persistence, manual replay still opens but can't save. That fail-closed behavior is a **safety net for the interim**, not an intended shipping state — don't describe shipping without the table as fine; get it applied before/with the next deploy.
...
- ⚠️ **Operational follow-ups:** (1) the `UserTourProgress` migration `20260923000000_add_user_tour_progress` is applied by the next Render API deploy (its buildCommand runs `prisma:migrate:deploy`) — until then `GET /tours/progress` 500s and tours fail closed (no auto-play). (2) **`tests/e2e/guided-tours.spec.ts` (6 tests) has never executed** — local Playwright Chromium revision mismatch; behaviours were verified live in Chrome instead. Run `pnpm exec playwright install chromium && pnpm e2e tests/e2e/guided-tours.spec.ts`. (3) Phase 2 (staff/admin tours + mounting `TourProvider` in the portal layout) needs its own plan. (4) Deferred minors: dialog-bail in the auto-play timer has no retry; a 401 save is dropped not queued; Back-button mid-tour can leave the overlay; `consumer-ticket-board.tsx` is 1,242 lines; stale `consumer-ticket-actions.spec.ts:89,99-102` assertions (pre-existing).
