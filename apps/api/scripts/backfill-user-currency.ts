...
async function main() {
  const users = await prisma.user.findMany({
    select: { id: true, phone: true, country: true, walletBalance: true },
  });
  let derived = 0;
  let grandfathered = 0;
  for (const u of users) {
    ...
    // ticket OR a non-zero wallet balance) keep PKR — their existing ledger is
    // PKR, and flipping them to USD would mislabel historical amounts. The
    // owner can convert specific grandfathered accounts manually if needed.
  ...
  console.log(
    `Backfill complete: ${derived} inactive users set to USD, ` +
      `${grandfathered} active users grandfathered to PKR (of ${users.length}).`,
  );
}
