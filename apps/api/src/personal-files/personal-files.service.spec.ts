  ...
  const tx = {
    personalFile: {
      create: jest.fn(async (a: { data: Record<string, unknown> }) => ({
        id: 'f1', ...a.data, createdAt: new Date(), deletedAt: null,
      })) as AnyMock,
      findUnique: jest.fn() as AnyMock,
      delete: jest.fn() as AnyMock,
    },
    userStorageUsage: { upsert: jest.fn() as AnyMock, update: jest.fn() as AnyMock },
  };
  return {
    personalFile: {
...
describe('PersonalFilesService', () => {
  ...
  let storage: { put: AnyMock; getSignedDownloadUrl: AnyMock; delete: AnyMock; exists: AnyMock };
  ...
  describe('upload', () => {
    ...
    it('rejects when quota would be exceeded', async () => {
      prisma.userStorageUsage.upsert.mockResolvedValue({
        bytesUsed: BigInt(500 * 1024 * 1024 - 32), fileCount: 100,
      });
      await expect(
    ...
    it('writes to storage and audit-logs on success', async () => {
      ...
      const r = await service.upload('u1', 'a@b', {
        buffer: PDF, originalName: 'order.pdf', declaredMime: 'application/pdf',
      });
  ...
  describe('signDownload', () => {
    ...
    it('returns signed URL and audit-logs on success', async () => {
      prisma.personalFile.findFirst.mockResolvedValue({
        id: 'f1', userId: 'u1', storageKey: 'k', displayName: 'order.pdf',
      });
  ...
  describe('softDelete', () => {
    ...
    it('sets deletedAt and audit-logs', async () => {
      prisma.personalFile.findFirst.mockResolvedValue({
        id: 'f1', userId: 'u1', displayName: 'x.pdf', sizeBytes: 1,
      });
  ...
  describe('restore', () => {
    ...
    it('restores within the window', async () => {
      const file = {
        id: 'f1', userId: 'u1', displayName: 'x.pdf',
        deletedAt: new Date(), createdAt: new Date(),
