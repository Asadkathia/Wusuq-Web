...
function normalizeLocationName(value: string) {
  return value
    ...
    .replace(/[/()-]/g, ' ')
    .replace(
      /\b(city|town|district|division|tehsil|cantt|cantonment|sharif|model)\b/g,
      ' ',
    )
    .replace(/[^a-z0-9 ]+/g, ' ')
    .replace(/\s+/g, ' ')
...
function pickDistrictForCity(
  ...
  const provinceHints = DISTRICT_CITY_HINTS[provinceName] ?? {};
  const provinceStations = RAW_POLICE_STATIONS_BY_PROVINCE[provinceName] ?? {};
  ...
  for (const districtName of districts) {
    ...
    const explicitHints =
      (provinceHints as Record<string, readonly string[]>)[districtName] ?? [];
