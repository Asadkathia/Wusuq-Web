...
describe('CaseTypesService', () => {
      ...
      findMany: jest.fn(async (args: any) => {
        ...
        return matched.sort((a, b) => {
          for (const clause of orderBy) {
            const [k, dir] = Object.entries(clause)[0] as [string, 'asc' | 'desc'];
            if (a[k] === b[k]) continue;
  ...
  it('returns the most-specific cohort when all filters match', async () => {
    const rows = [
      { courtLevel: 'Lower Court', subCourt: 'Sessions Court', district: 'Lahore', region: 'Punjab', highCourtCode: null, code: 'X', label: 'X', source: 'dsj', priority: 0, isActive: true },
      { courtLevel: 'Lower Court', subCourt: 'Sessions Court', district: null, region: null, highCourtCode: null, code: 'Y', label: 'Y', source: 'fallback', priority: 0, isActive: true },
    ];
  ...
  it('falls back when district has no rows but subCourt does', async () => {
    const rows = [
      { courtLevel: 'Lower Court', subCourt: 'Family Court', district: null, region: null, highCourtCode: null, code: 'F', label: 'Family', source: 'fallback', priority: 0, isActive: true },
    ];
    const svc = new CaseTypesService(mkPrisma(rows));
  ...
  it('falls back to courtLevel when no rows match subCourt', async () => {
    const rows = [
      { courtLevel: 'High Court', subCourt: null, district: null, region: null, highCourtCode: null, code: 'WP', label: 'Writ Petition', source: 'fallback', priority: 0, isActive: true },
    ];
    const svc = new CaseTypesService(mkPrisma(rows));
  ...
  it('orders by priority desc then label asc', async () => {
    const rows = [
      { courtLevel: 'Supreme Court', subCourt: null, district: null, region: null, highCourtCode: null, code: 'B', label: 'B', source: 'scp', priority: 1, isActive: true },
      { courtLevel: 'Supreme Court', subCourt: null, district: null, region: null, highCourtCode: null, code: 'A', label: 'A', source: 'scp', priority: 1, isActive: true },
      { courtLevel: 'Supreme Court', subCourt: null, district: null, region: null, highCourtCode: null, code: 'C', label: 'C', source: 'scp', priority: 5, isActive: true },
    ];
    const svc = new CaseTypesService(mkPrisma(rows));
