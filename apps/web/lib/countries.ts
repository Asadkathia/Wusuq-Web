...
/**
 * Flag emoji from an ISO 3166-1 alpha-2 code — the two regional-indicator
 * symbols. Computed (not hand-typed) so the full country list can't carry a
 * mistyped flag.
 */
...
// [ISO alpha-2, display name, E.164 dial code] for every country/territory.
// Kept as tuples so the flag is derived once below.
...
const ALL = COUNTRY_DATA.map(([code, name, dial]) => ({
  code,
  name,
...
// Pakistan pinned first (the default market); everyone else alphabetical. The
// picker is searchable, so completeness matters more than curated ordering.
export const COUNTRIES: Country[] = [
  ALL.find((c) => c.code === DEFAULT_COUNTRY_CODE)!,
  ...ALL.filter((c) => c.code !== DEFAULT_COUNTRY_CODE).sort((a, b) =>
